from test.pages.common import DeviceDelete, DeviceList, PasswordChange
    ...
    def setUp(self) -> None:
        ...
        self.first_device = self.first_group.device_set.create(
            ...
            uid=Device.generate_device_uid(
                f"{self.first_member.username}-{self.first_group.name}-first device"
            ),
        )
        ...
        self.second_device = self.first_group.device_set.create(
            name="second device",
            uid=Device.generate_device_uid(
                f"{self.first_member.username}-{self.first_group.name}-second device"
            ),
        )

        self.second_member = create_member(**SECOND_MEMBER)
        self.second_group = self.second_member.devicegroup_set.create(
            **SECOND_DEVICE_GROUP
        )
        self.third_device = self.second_group.device_set.create(
            name="third device",
            uid=Device.generate_device_uid(
                f"{self.second_member.username}-{self.first_group.name}-third device"
            ),
        )
...
class TestDeviceDeleteView(BaseDeviceDeleteTestCase):
    ...
    def test_delete_device_remove_device(self) -> None:
        ...
        self.assertRedirects(
            response, DeviceList.get_url(), 302, 200, fetch_redirect_response=True
        )
