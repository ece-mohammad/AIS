...
class TestDeviceCreateForm(BaseDeviceTestCase):
    ...
    def test_device_create_fields_group(self):
        ...
        self.assertEqual(
            self.group_field.queryset.first().name, FIRST_DEVICE_GROUP["name"]
        )
...
class TestDeviceCreateView(BaseDeviceTestCase):
    def test_create_device_adds_device_to_group_and_member(self):
        ...
        response = self.client.post(
            ...
            data=dict(
                ...
                group=DeviceGroup.objects.get(
                    name=FIRST_DEVICE_GROUP["name"], owner=self.first_member
                ).pk,
            ),
            follow=True,
        )
        ...
        self.assertRedirects(
            response,
            DeviceDetails.get_url(device_uid=device.uid),
            302,
            200,
    ...
    def test_create_device_same_name_different_member(self):
        ...
        new_device = self.first_member.devicegroup_set.first().device_set.get(
            name=device.name
        )
        self.assertRedirects(
            response,
            DeviceDetails.get_url(device_uid=new_device.uid),
            302,
            200,
            fetch_redirect_response=True,
        )
        self.assertEqual(
            self.first_member.devicegroup_set.first().device_set.count(), 1
        )
        self.assertEqual(
            self.second_member.devicegroup_set.first().device_set.count(), 1
        )
        self.assertEqual(
            self.first_member.devicegroup_set.first().device_set.first().name,
            device.name,
        )
