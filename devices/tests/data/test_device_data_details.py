...
class TestDeviceDataRendering(BaseDeviceDataDetailsTestCase):
    ...
    def test_base_device_data_setup(self):
        ...
        self.assertEqual(
            self.first_member.devicegroup_set.count(), len(FIRST_MEMBER_GROUPS)
        )
        self.assertEqual(
            self.first_member.devicegroup_set.first().name, FIRST_MEMBER_GROUPS[0].name
        )
        self.assertEqual(
            self.first_member.devicegroup_set.last().name, FIRST_MEMBER_GROUPS[1].name
        )
        self.assertEqual(
            self.first_member.devicegroup_set.first().device_set.count(),
            len(FIRST_MEMBER_DEVICES) // 2,
        )
        self.assertEqual(
            self.first_member.devicegroup_set.last().device_set.count(),
            len(FIRST_MEMBER_DEVICES) // 2,
        )
        self.assertDictContainsSubset(
            dict(device_count=4),
            Member.objects.filter(pk=self.first_member.pk)
            .annotate(device_count=Count("devicegroup__device", unique=True))
            .values()[0],
        )
        self.assertEqual(DeviceData.objects.count(), len(FIRST_DEVICE_DATA))
        self.assertEqual(
            Device.objects.first().devicedata_set.count(), len(FIRST_DEVICE_DATA)
        )
...
class TestDeviceDataDetailsView(BaseDeviceDataDetailsTestCase):
    ...
    def test_device_data_details_redirects_anonymous_user(self):
        client = Client()
        response = client.get(DeviceDataDetails.get_url(data_id=1), follow=True)
    ...
    def test_device_data_details_shows_data_message(self):
        response = self.client.get(DeviceDataDetails.get_url(data_id=1), follow=True)
