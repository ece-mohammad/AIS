...
class BaseDeviceGroupsAPIViewDetailsTestCase(BaseDeviceGroupsAPIViewListTestCase):
    def setUp(self):
        ...
        self.device_group = DeviceGroupSerializer(
            self.first_member.devicegroup_set.first(), context={"request": request}
        )
        self.url = reverse_lazy(
            "api:group_details", kwargs=dict(pk=self.device_group["id"].value)
        )
        return ret
...
class TestDeviceGroupsViewAPIDetails(BaseDeviceGroupsAPIViewDetailsTestCase):
    def test_api_device_group_details_get_device_group(self):
        ...
        self.assertEqual(
            response.data["description"], self.device_group["description"].value
        )
        self.assertEqual(response.data["owner_id"], self.device_group["owner_id"].value)
        self.assertEqual(
            response.data["creation_date"], self.device_group["creation_date"].value
        )
