...
from api.v2.serializers import (DeviceDataSerializer, DeviceGroupSerializer,
                            DeviceSerializer, MemberSerializer)
...
class DeviceGroupListAPIView(AuthenticatedUserAPIView):
    ...
    def get(self, request: Request, format = None) -> Response:
        ...
        owner = request.user
        serializer = DeviceGroupSerializer(DeviceGroup.objects.filter(owner=owner), many=True, context={"request": request})
        return Response(serializer.data)
    ...
    def post(self, request: Request, format = None) -> Response:
        ...
        serializer = DeviceGroupSerializer(data=group_data, context={"request": request})
...
class DeviceListAPIView(AuthenticatedUserAPIView):
    ...
    def get(self, request: Request, format = None) -> Response:
        ...
        owner = request.user
        serializer = DeviceSerializer(Device.objects.filter(group__owner=owner), many=True, context={"request": request})
        return Response(serializer.data)
...
class DeviceDataListAPIView(AuthenticatedUserAPIView):
    ...
    def get(self, request: Request, device_uid: str, format = None) -> Response:
        ...
        device_data = DeviceData.objects.filter(device__uid=device_uid, device__group__owner=self.request.user)
        serializer = DeviceDataSerializer(device_data, many=True, context={"request": request})
        return Response(serializer.data)

    def post(self, request: Request, device_uid: str, format = None) -> Response:
        """Add data to device"""
        device = get_object_or_404(Device, uid=device_uid, group__owner=self.request.user)
        device_data = request.data
        device_data["device_id"] = device.id
        serializer = DeviceDataSerializer(data=device_data, device_uid=device_uid, context={"request": request})
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data, status=status.HTTP_201_CREATED)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
